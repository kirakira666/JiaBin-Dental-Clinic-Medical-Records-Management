...
export default {
  ...
  methods: {
    ...
    touchstart (e) {
      ...
      const pageX = `${boxStep}> ${-endWidth} && ${boxStep} < 0?${boxStep}:(x+${this.x} < 0? ${-endWidth}:0)`
      ...
      for (let i = 0; i < this.options.length; i++) {
        ...
        const pageButtonX = `${moveX}&& (x+${this.x} > ${-endWidth})?${moveX}:${-moveMix}`
        props.push({
    ...
    animation (type) {
      ...
      const exit = `t>${time}`
      const translate_x_expression = `easeOutExpo(t,${this.x},${type ? (-endWidth - this.x) : (-this.x)},${time})`
      const props = [{
        element: this.selector,
        property: 'transform.translateX',
      ...
      for (let i = 0; i < this.options.length; i++) {
        ...
        const moveX = `(${step}) * ${moveMix}`
        const pageButtonX = `easeOutExpo(t,${moveX},${type ? -moveMix + '-' + moveX : 0 + '-' + moveX},${time})`
        props.push({
          element: buttonSelectors,
          property: 'transform.translateX',
          expression: pageButtonX
